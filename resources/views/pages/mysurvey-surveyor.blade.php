	...
	<div class="row">
		<div class="col-md-12 col-lg-12 col-xl-12">
			...
			<?php 
				...
				,'users.id')->where('email_verify','1')->where('users.created_by',$user->id)->get();
			...
			?>
			@if($user->type=='2' && count($survey_rec)!=0)
					...
					<div class="row">
						...
						{!! Form::open(array('url' => '/mysurvey', 'method' => 'post','name'=>'mySearchForm','files'=>true,'novalidate' => 'novalidate','id' => 'mySearchForm')) !!}
								<div id="upcomming_filter">
									<div class="row">
										<div class="col-md-3">
												{!! Form::select('surveyor_id',$surveyoruser_box,$surveyor_id, ['class' => 'form-control','required'=>'required','onkeypress' => 'error_remove()']) !!}
										</div>
			...
			@endif
				<div class="surveys-list">
                            <div class="list-group" id="myList" role="tablist">
                                ...
                                <a class="list-group-item list-group-item-action" data-toggle="list" href="#past" role="tab">Past({{$pending_payment_survey_data->count()+$received_payment_survey_data->count()+$paid_survey_data->count()}})</a>
                            </div>
                            <div class="tab-content">
															...
															<span class="ship-info">
																...
																<a href="#" class="paid pending"> {{$helper->GetSurveyStatusBykey($data->status)}}</a>
																	<?php if($data->status=='1' || $data->status=='3')
																	{
																			...
																			if($operator_id==$user->id){$sender_id=$operator_id;$receiver_id=$surveyor_id;}else{$sender_id=$surveyor_id;$receiver_id=$operator_id;}
																			//$chat=new App\Models\Chat;
																			//$chat_unread_count=$chat->select('chat.id')->where('survey_id',$data->id)->where('is_read','0')->where('receiver_id',$user->id)->count();
																		?>
                                ...
                                <div class="tab-pane" id="past" role="tabpanel">
                                    <div class="list-group" id="myListpastsurveyor" role="tablist">
                                        <a class="list-group-item list-group-item-action active" data-toggle="list" href="#pendingpaymentpast" role="tab">Pending Payment ({{$pending_payment_survey_data->count()}})</a>
                                        <a class="list-group-item list-group-item-action" data-toggle="list" href="#receivedpaymentpast" role="tab">Payment Received ({{$received_payment_survey_data->count()}})</a>
                                    ...
                                    <div class="tab-content">
                                        <div class="tab-pane active" id="pendingpaymentpast" role="tabpanel">
										<ul class="upcomeing-past-list">
												<?php if(count($pending_payment_survey_data)!=0 ) 
												{?>

													@foreach($pending_payment_survey_data as $data)  
													<li>
														<span class="past-img">
															<img src="{{ URL::asset('/public/media') }}/ship.png" alt="#">
														</span>
                                        ...
                                        <div class="tab-pane" id="receivedpaymentpast" role="tabpanel">
										<ul class="upcomeing-past-list">
												<?php if(count($received_payment_survey_data)!=0 ) 
												{?>

													@foreach($received_payment_survey_data as $data)  
													<li>
														<span class="past-img">
															<img src="{{ URL::asset('/public/media') }}/ship.png" alt="#">
														</span>
													...
													@endforeach
														...
														<?php }else{ ?>
																		...
																		<li>
																			<span class="ship-info">
																				<h3 class="text-center">No paid surveys building up your account balance…</h3>
																			</span>
																		</li>
															<?php  } ?>
												</ul>
												...
												{?>
													...
													@endforeach
														...
														<?php }else{ ?>
																		...
																		<li>
																			<span class="ship-info">
																				<h3 class="text-center">No past surveys you have received funds for... </h3>
																			</span>
																		</li>
															<?php  } ?>
												</ul>
