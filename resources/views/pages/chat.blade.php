<script type="text/javascript">
    function formatDate(dt) {
        ...
        return year+'-'+month+'-'+date+' '+formatAMPM(dt);
        //console.log(year+'-'+month+'-'+date+' '+formatAMPM(dt));
		}
    ...
    { 
      ...
      //                    $('#viewchat_'+data.survey_id).html('<img src="https://72.octallabs.com/imars/public/media/massage_icon.png" />')
      //                 } 
      //             }); 
     ...
     // alert(current_time);
      if(parseInt(senderID)>parseInt(receiverID))
      {
        ...
        var chat_node     =  receiverID+"_"+senderID;
           ...
           // alert(survey_id);
				firebase.database().ref('ChatHistory/'+survey_id+'/'+chat_node).on('value', resp => {
   ...
   function  myFunction(){
			...
			if(senderMessage != '' && senderMessage != null && senderMessage != undefined)
      {
        ...
        $.ajax({  dataType: 'json',
                      type:'POST', 
                      url:'{{ URL::to('/api/operator/chat-emailw') }}', 
                      data: {sender_id: senderID,"receiver_id":receiverID, message: senderMessage},
                     // data:'sender_id='+senderID+'&receiver_id='+receiverID+'&message='+senderMessage, 
                      success:function(html){ 
				...
				firebase.database().ref('ChatHistory/'+survey_id+'/'+chat_node).push().set({
  ...
  <div class="form-container">
   ...
   ?>
    <div class="massage_outer">
      ...
      <span class="send_btn"><button class="sendMsgBtn" id="sendMsgBtn"><img src="{{ URL::asset('/public/media') }}/send.png" align=""></button></span>
    </div>
    <?php } ?>
  </div>
</div>
