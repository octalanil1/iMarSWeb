				...
				$r=1;
						...
						{		
							$survey_count =  Survey::where('user_id',$users->user_id)->count();
								...
								{
									$first = DB::table('custom_survey_users')->select(DB::raw('AVG(TIMESTAMPDIFF(SECOND,updated_at,created_at)) as recponce_time'))->where('surveyors_id',$users->id);
									$responce = DB::table('survey_users')->select(DB::raw('AVG(TIMESTAMPDIFF(SECOND,updated_at,created_at)) as recponce_time'))->where('surveyors_id',$users->id)->union($first)->first();
					...
					//	dd($surveyor_data );
							foreach($surveyor_data  as $surveyor )
							{
								...
								if( $surveyor_event_data!=$users_count)
								{
									...
									$helper=new Helpers;
									$pricing1=$helper->SurveyorPrice($decoded['survey_category_id'],$surveyor->id);						
				...
				$country_data=  Port::select('country_id')->where('id',$port_id)->first();
						...
						foreach($usersdata1 as $key => $value)
						{
							...
							$surveyuser->save();
					                    ...
					                    $message_token =  SurveyUsers::select('users.id','users.device_id','users.type','users.country_id')
		...
		if($decoded) 
		{
			...
			if(!empty($decoded['surveyors_id']) && !empty($decoded['survey_id']) && !empty($decoded['type']))
			{
				...
				if(!empty($surveyor_data) && !empty($survey_users))
				{
					...
					$operator_token =  User::select('users.id','users.device_id','users.type','users.country_id')->where('id',$surveyor_data->user_id)->first(); 
					...
					{   $survey_users->status="upcoming";
						...
						$notification->noti_type = 'Accept Your Survey Request';
					...
					}else
					{
						...
						$notification->noti_type = 'Decline Your Survey Request';
						$notification->user_type = $operator_token->type;
						...
						if(!empty($survey_users_next))
						{
							...
							$message_token =  User::select('users.id','users.device_id','users.type','users.country_id')->where('id',$survey_users_next->surveyors_id)->first(); 
							$helper=new Helpers;
...
$message = 'One or more required fields are missing. Please try again.';
					...
					//if($surveyor_data_count > 0){
						...
						//{    
						   if($surveyor_data->image!="")
								...
								{
									$report = url('/').'/media/report/'.$surveyor_data->report;
								}else{
									$report ='';
								}
								if($surveyor_data->invoice !="")
								{
									$invoice = url('/').'/media/invoice/'.$surveyor_data->invoice;
								}else{
									$invoice ='';
								}
						...
						if($surveyor_data->survey_type_id=='31')
						{
							...
							$bid_count = DB::table('custom_survey_users')->where('survey_id',$surveyor_data->id)->where('surveyors_id',$user_id)->where('status','upcoming')->count();
								if($bid_count>0){
								$bid_status='1';
								}else{
									$bid_status='0';
		...
		if (!empty($decoded['user_id'])) 
		{
				   ...
				   ->where(function ($query)  {
					...
					->where(function ($query)  use ($userid)
					{
						$query->Where('custom_survey_users.surveyors_id',$userid)
										  ...
										  ->orwhere('survey.assign_to',$userid );
					  })
			...
			if(!empty( $surveyor_data) && count ($surveyor_data)>0)
			{
	               ...
	               $data=array();
					...
					$is_avail = (string)$user_data->is_avail;
	                $status = 1;
   ...
   public function UserSurveyType(Request $request)
		...
		public function reportsubmit(Request $request)
		{
			...
			else
			{
				...
				//	dd($surveyor);
					...
					{
						$image = url('/').'/media/users/'.$surveyor->report;
					}else
					{
						$image ='';
					}
			...
			if (!empty($decoded['operator_id']) && !empty($decoded['surveyor_id']) && !empty($decoded['survey_id'])) 
				{
					...
					if(!empty($surveyor_data) || !empty($survey_users))
					{
						$surveyor_token =  User::select('users.id','users.email','users.type','users.device_id','users.country_id')->where('id',$surveyor_data->accept_by)->first(); 
	...
	public function SurveyFilterUser(Request $request)
	{
		...
		if (!empty($decoded['user_id'])) 
		{
			$user_data =  User::select('*')->where('status','!=','0')
			...
			if(!empty($user_data) )
			{
				$cuser_data =  User::select('*')->where('status','!=','0')
				->where('id',$decoded['user_id'])
				->first();
				$data1[]=array('id'=>(string)($cuser_data->id),
				'name'=>$cuser_data->first_name.' '.$cuser_data->last_name,
				'mobile'=>(string)$cuser_data->mobile);
			
					foreach($user_data  as $data )
					{
						$ip=$data->id;
						if($cuser_data->type=='2')
						{
							$ssurveys=Survey::select('survey.*')
							->where(function ($query) use ($ip) {
								$query->whereRaw("find_in_set($ip,surveyors_id)")
									->orwhere('assign_to',$ip );})->get();
						}else{
								$ssurveys=Survey::select('survey.*')
								->where(function ($query) use ($ip) {
									$query->whereRaw("find_in_set($ip,user_id)")
									 ->orwhere('assign_to_op',$ip );})->get();
						}
	...
	public function CustomSurveyUsersList(Request $request)
	{
		...
		if (!empty($decoded['survey_id'])) 
		{
			...
			if($csurvey_users!="0")
			{
				$csurvey_users=User::select('users.id',
				DB::raw('CONCAT(users.first_name, "  ", users.last_name) as username')
				,'users.mobile','users.country_code','custom_survey_users.amount')
				->leftJoin('custom_survey_users', 'custom_survey_users.surveyors_id', '=', 'users.id')
				   ->where('custom_survey_users.survey_id',$decoded['survey_id'])
	...
	public function AssigntosurveyorList(Request $request)
	{
		...
		if (!empty($decoded['user_id'])) 
		{
			$surveyor_list_count =User::select(DB::raw('CONCAT(users.first_name, "  ", users.last_name) as username')
						,'users.id')->where('email_verify','1')->where('is_signup','1')->where('status','1')
		...
		if (!empty($decoded['user_id']) && !empty($decoded['survey_id']) && !empty($decoded['surveyor_id'])) 
		{
			$surveyor_data =  Survey::select('survey.*')->where('survey.id',$decoded['survey_id'])->first();
						...
						if(!empty($surveyor_data) )
						{
							...
							$surveyors_token =  User::select('users.id','users.email','users.type','users.device_id','users.country_id')->where('id',$decoded['surveyor_id'])->first(); 
		...
		if (!empty($decoded['user_id']) && !empty($decoded['survey_id']) && !empty($decoded['operator_id'])) 
		{
			$surveyor_data =  Survey::select('survey.*')->where('survey.id',$decoded['survey_id'])->first();
						...
						if(!empty($surveyor_data) )
						{
							...
							$helper=new Helpers;
							$opeartor_token =  User::select('users.id','users.email','users.type','users.device_id','users.country_id')->where('id',$decoded['operator_id'])->first(); 
			...
			if(!empty($decoded['user_id']) && !empty($decoded['survey_id']))
			{
				...
				if(!empty($surveyor_data) )
				{
					...
					$surveyor_data->status="2";
						...
						{
							$surveyors_token =  User::select('device_id')->where('id',$surveyor_id->surveyors_id)->first(); 

							$helper->SendNotification($surveyors_token->device_id,'Cancel  Survey','This Survey has been cancel by opeartor');

						}

						if($surveyor_data->assign_to!=0)
							 {
								$surveyors_token =  User::select('users.id','users.email','users.type','users.device_id','users.country_id')->where('id',$surveyor_data->assign_to)->first(); 

							 }else
							 {
								$surveyors_token =  SurveyUsers::select('users.id','users.email','users.type','users.device_id','users.country_id')
								->leftJoin('users', 'survey_users.surveyors_id', '=', 'users.id')
								->where('survey_users.survey_id',$decoded['survey_id'])
						...
						$status=1;
						$message = 'Survey cancel successfully.';
				}else{
					$message = 'Data Not Found.';
				}
	...
	{   $user_id =  (string)($request->input('user_id'));
		...
		if(!empty($user_id))
		{
				...
				if($user_d->type =="0" || $user_d->type =="1")
						{
							...
							if($operator_id !="")
							{
								$upcoming_survey_data=$upcoming_survey_data->where(function ($query) use ($operator_id) {
									$query->Where('survey.user_id',$operator_id)
									->orWhere('survey.assign_to_op',$operator_id);});	
			...
			if($user_d->type =="0" || $user_d->type =="1")
			{
				...
				if($operator_id !="")
				{
					$past_survey_data=$past_survey_data->where(function ($query) use ($operator_id) {
						$query->Where('survey.user_id',$operator_id)
						->orWhere('survey.assign_to_op',$operator_id);});	
	...
	public function reportissuepost(Request $request)
	{
		...
		$validator = Validator::make($request->all(), [
			...
			}else
			{
				...
				$status=1;
				$message="You have successfully created repport an issue.";
